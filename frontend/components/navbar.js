...
export default function NavBar({ page }) {
    ...
    return (
        ...
        <nav id="navbar" className="fixed flex h-fit justify-center items-center w-full pt-5 overflow-auto transition ease-in"> 
            ...
            <ul className="bg-cool opacity-90 rounded-lg flex justify-center items-center">
                {
                    navbarlinks.map((current) => {
                        return current.name === page? (
                            <li id="current-tab" key={current.name} className='text-subtitle px-6  leading-[50px] text-lg hover:transition hover:ease-in duration-200'>
                                    <Link href={current.href} className='p-2 rounded-md'>{current.name}</Link>
                            </li>
                            )
                            :
                            <li id="nav-item" key={current.name} className="text-subtitle px-6  leading-[50px] text-lg hover:transition hover:ease-in duration-200">
                                <Link href={current.href} className='p-2 rounded-md'>{current.name}</Link>
                            </li>
